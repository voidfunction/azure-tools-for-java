...
public class WAStoragePublisher extends Recorder {
	...
	public static final class WAStorageDescriptor extends
				...
				for (StorageAccountInfo sa: storageAccounts) {
					if (sa.getStorageAccName().equals(name)) {
						...
						if (storageAccountInfo != null) {
							storageAccountInfo.setBlobEndPointURL(Utils.getBlobEP(
									storageAccountInfo.getStorageAccName(),
									storageAccountInfo.getBlobEndPointURL()));
						}
						break;
					}
		...
		/* public ComboBoxModel doFillContainerNameItems(
			...
			List<String> containerList = getContainersList(storageAccName);
			if (containerList != null) {
				for (String containerName : getContainersList(storageAccName)) {
					m.add(containerName);
				}
			}
			return m;
