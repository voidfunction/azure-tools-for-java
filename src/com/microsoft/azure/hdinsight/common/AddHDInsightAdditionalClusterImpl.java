...
public class AddHDInsightAdditionalClusterImpl {
    ...
    public static StorageAccount getStorageAccount(String clusterName, String storageName, String storageKey, String userName, String password) throws HDIException {
        ...
        StorageAccount account = new StorageAccount(storageName, storageKey, true, defaultContainer);

        //getting container to check the storage key is correct or not
        try {
            StorageClientImpl.getInstance().getBlobContainers(account);
        } catch (HDIException e) {
            throw new HDIException("Invalid Storage Key");
        }
